import { assert, assertUsage, castProp, hasProp, higherFirst, normalizeBaseUrl, normalizePath, prependBaseUrl, slice, } from '../../shared/utils';
import { getPreloadUrls } from '../getPreloadTags';
import { getSsrEnv } from '../ssrEnv';
import { getViteManifest } from '../getViteManifest';
// import { isAbsolute } from 'path';
import { inferMediaType } from './inferMediaType';
...
async function getPageAssets(pageContext, dependencies, pageClientFilePath, isPreRendering) {
    ...
    const preloadAssets = await getPreloadUrls(pageContext, dependencies, clientManifest, serverManifest);
...
function sortPageAssetsForHttpPush(pageAssets) {
    pageAssets.sort(higherFirst(({ assetType, preloadType }) => {
        let priority = 0;
...
function retrieveViteManifest(isPreRendering) {
    ...
    assertUsage(clientManifest && serverManifest, 'You are ' +
        userOperation +
...
async function injectAssets__public(htmlString, pageContext) {
    assertUsage(typeof htmlString === 'string', '[injectAssets(htmlString, pageContext)]: Argument `htmlString` should be a string.');
    ...
    castProp(pageContext, '_getPageAssets');
...
async function injectAssetsAfterRender(htmlString, pageContext) {
    ...
    assertUsage(!injectPageInfoAlreadyDone(htmlString), 'Assets are being injected twice into your HTML. Make sure to remove your superfluous `injectAssets()` call (`vite-plugin-ssr` already automatically calls `injectAssets()`).');
