...
function compilePath(path, caseSensitive = false, end = true) {
    assertWarning(path === '*' || !path.endsWith('*') || path.endsWith('/*'), `Route path "${path}" will be treated as if it were ` +
...
function safelyDecodeURIComponent(value, paramName) {
    try {
        ...
        assertWarning(false, `The value for the URL param "${paramName}" will not be decoded because` +
